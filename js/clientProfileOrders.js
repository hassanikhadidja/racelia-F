...
export function renderProfileOrdersList(orders = getProfileOrders()) {
  if (!orders.length) {
    return `<p class="profile-orders-empty">No orders yet. Your purchases will appear here.</p>`;
  }
    ...
    .map((order) => {
      const promoHtml = order.promoUsed
        ? `<p class="profile-order__promo">
            <span class="profile-order__promo-label">Promo used</span>
        ...
        : `<p class="profile-order__promo profile-order__promo--none">No promo code used</p>`;
...
export function getProfileOrdersOverlayMarkup() {
  ...
  <div class="profile-orders-sheet profile-sheet" role="dialog" aria-labelledby="profile-orders-title">
    <div class="profile-sheet-header">
      <h3 id="profile-orders-title">My orders</h3>
      <button type="button" class="profile-sheet-close" data-close="profile-orders-overlay">Close</button>
    </div>
